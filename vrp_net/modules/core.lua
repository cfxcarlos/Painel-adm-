...
function vRP.HasService(Passport,Permission)
    if Groups[Permission] then
        for Number,v in pairs(Groups[Permission].Permission) do
            if Groups[Number].Service[tostring(Passport)] then
                return true
            end
        end
    end
...
function tvRP.invUpdate(Slot,Target,Amount)
    ...
    if Item and (Target == "104" or Slot == "104") then
        TriggerClientEvent("inventory:Update",source)
        local Skinshop = ItemSkinshop(Item)
